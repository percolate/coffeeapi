...
class CoffeeHandler(webapp2.RequestHandler):
    def get(self):
        if valid_api_key(self.request):
            write_json_response(self.response, [c.to_json_summary_dict() for c in Coffee.query().fetch()])
        else:
...
class CoffeeItemHandler(webapp2.RequestHandler):
    def get(self, id):
        if valid_api_key(self.request):
            write_json_response(self.response, Coffee.get_by_id(id=id).to_json_full_dict())
        else:
            write_api_error_response(self.request, self.response)
...
def valid_api_key(request):
    return request.get("api_key") == API_KEY or request.headers.get('Authorization') == API_KEY


def write_api_error_response(request, response):
    if request.get("api_key", None) == None and request.headers.get('Authorization', None) == None:
